...
export interface Order {
  ...
  // Optional courier identifiers
  awbCode?: string;
  courierName?: string;
  // Tracking snapshot written by backend
  tracking?: {
    status?: string;
...
// Upsert tracking information and identifiers for an order in both global and customer nodes
export const updateOrderTracking = async (
  orderId: string,
  tracking: NonNullable<Order['tracking']>,
  identifiers?: { awbCode?: string; courierName?: string; shiprocketShipmentId?: string }
): Promise<void> => {
  try {
    const orderRef = ref(database, `orders/${orderId}`);
    const orderSnapshot = await get(orderRef);
    ...
    if (identifiers?.awbCode) updateData.awbCode = identifiers.awbCode;
    if (identifiers?.courierName) updateData.courierName = identifiers.courierName;
    if (identifiers?.shiprocketShipmentId) updateData.shiprocketShipmentId = identifiers.shiprocketShipmentId;
