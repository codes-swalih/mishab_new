...
const Checkout = () => {
  ...
  const [isPaymentProcessing, setIsPaymentProcessing] =
    useState<boolean>(false);
  const [taxDataCache, setTaxDataCache] = useState<{
    [taxId: string]: TaxType;
  }>({});
  const [taxLoadingCache, setTaxLoadingCache] = useState<{
    [taxId: string]: boolean;
  }>({});
  ...
  const fetchTaxData = React.useCallback(
    async (taxId: string) => {
  ...
  const populateFormWithAddress = React.useCallback((address: Address) => {
    ...
    const country = locationsData.countries.find(
      (c) => c.name === (address.country || "India")
    );
    if (country) {
  ...
  const handleAddressSelect = (address: Address) => {
    ...
    const shippingCost = getShippingCostForCitySync(
      address.city,
      address.state,
      shippingData,
      address.country
  ...
  const handleSaveAddressOnly = async () => {
    ...
    try {
      ...
      showMessage(
        "error",
        `Failed to save/update address. Error: ${
  ...
  const totalShippingCost = isFreeShippingApplied
    ? 0
    : (currentShippingCost || 0) * displayCartItems.length;
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      const { valid, reason, discount, isFreeShipping } = computeCouponDiscount(
        calculatedTotalCart,
        record
      );
      if (!valid) {
  ...
  const createOrderData = (
    ...
    const orderItems: OrderItem[] = displayCartItems.map((item) => {
      ...
      if (
        (item as any).salePrice !== undefined &&
        (item as any).salePrice !== null
      ) {
      ...
      if (
        (item as any).commissionAmount !== undefined &&
        (item as any).commissionAmount !== null
      ) {
    ...
    const missingAddressFields = requiredAddressFields.filter(
      (field) => !orderAddress[field as keyof OrderAddress]
    );
    ...
    const undefinedKeys = Object.keys(orderData).filter(
      (key) => orderData[key] === undefined
    );
    if (undefinedKeys.length > 0) {
      throw new Error(
        `Order data contains undefined values: ${undefinedKeys.join(", ")}`
      );
    }
  ...
  const createIndividualOrders = async (
    ...
    razorpayPaymentId?: string,
    razorpayOrderId?: string
  ): Promise<Omit<Order, "id" | "createdAt" | "updatedAt">[]> => {
    if (!user?.uid) {
      throw new Error("User must be logged in to create an order");
    }
    ...
    console.log(
      "Cart items in createIndividualOrders:",
    ...
    const totalItems = displayCartItems.reduce(
    ...
    for (let index = 0; index < displayCartItems.length; index++) {
      ...
      if (item.vendor) {
        try {
          ...
          console.error(
            "Error fetching vendor info for product:",
            item.id,
            error
          );
        }
      ...
      const orderItem: OrderItem = {
        ...
        ...((item as any).commissionAmount !== undefined &&
          (item as any).commissionAmount !== null && {
            commissionAmount: (item as any).commissionAmount,
          }),
        ...(vendorInfo && {
          vendorName:
            vendorInfo.businessName || vendorInfo.storeName || vendorInfo.name,
          vendorEmail: vendorInfo.email,
        }),
      };

      // Calculate total commission for this individual order (single item)
      const itemTotalCommission =
        ((item as any).commissionAmount || 0) * item.quantity;
  ...
  const handleRazorpayPayment = async () => {
    ...
    await initiatePayment(
      ...
      async (response) => {
        ...
        try {
          // Create individual orders for each product
          individualOrders = await createIndividualOrders(
            "razorpay",
            response.razorpay_payment_id,
            response.razorpay_order_id
          );
          const parentOrderId =
            individualOrders[0]?.parentOrderId || response.razorpay_order_id;
          const savedOrderIds = await saveIndividualOrders(
            parentOrderId,
            individualOrders
          );
          ...
          clearCart();
          router.push(
            `/payment/success?payment_id=${response.razorpay_payment_id}&order_id=${parentOrderId}&amount=${finalAmount}`
          );
        } catch (error) {
          // Error saving order
          showMessage(
            "error",
            `Payment successful but failed to save order. Error: ${
              error instanceof Error ? error.message : "Unknown error"
            }. Please contact support with order ID: ${
              response.razorpay_order_id
            }`
          );
          clearCart();
          router.push(
            `/payment/success?payment_id=${response.razorpay_payment_id}&order_id=${response.razorpay_order_id}&amount=${finalAmount}`
          );
        } finally {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (activePayment === "razorpay") {
      ...
      try {
        ...
        const totalAmount = individualOrders.reduce(
        ...
        router.push(
          `/payment/success?order_id=${parentOrderId}&amount=${totalAmount}&payment_method=cod`
        );
      } catch (error) {
        // Error saving COD order
        showMessage(
          "error",
          `Failed to save COD order. Error: ${
            error instanceof Error ? error.message : "Unknown error"
          }. Please try again.`
        );
      } finally {
  ...
  return (
    <>
      ...
      <div className="cart-block md:py-20 py-10">
        <div className="container">
          <div className="content-main flex justify-between max-xl:flex-col g">
                                ...
                                {!address.isDefault && (
                                  <button
                                    ...
                                    onClick={async (e) => {
                                      e.stopPropagation(); // Prevent address selection
                                      try {
                                        const success = await makeDefault(
                                          address.id!
                                        );
                                        if (success) {
                                          showMessage(
                                          ...
                                          showMessage(
                                            "error",
                                            "Failed to set as default. Please try again."
                                          );
                                        }
                                      } catch (error) {
                    ...
                    <div className="address-form">
                      <div className="grid sm:grid-cols-2 gap-4 gap-y-5 flex-wrap">
                        <div className="">
                          ...
                          <input
                            className="broder border-gray-400 px-4 py-3 w-full rounded-lg"
                            id="firstName"
                            type="text"
                        ...
                        <div className="">
                          ...
                          <input
                            className="broder border-gray-400 px-4 py-3 w-full rounded-lg bg-gray-100"
                            id="country"
                            name="country"
                            value="India"
                            readOnly
                        ...
                        <div className="select-block">
                          ...
                          rounded-lg border border-1 border-gray-400 pr-4"
                          >
                            <select
                              className="broder border-gray-400 px-4 py-3 w-full rounded-lg bg-white"
                              id="state"
                              name="state"
                              value={selectedState}
                            ...
                            >
                              <option value="">
                                Select State (Debug: {availableStates.length}{" "}
                                states)
                              </option>
                ...
                <div className="coupon-input-block mt-4">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1.  h-12 bg-surface pl-4 pr-4 rounded-lg border border-gray-400"
                    />
                    <button
                      type="button"
                      onClick={handleApplyCoupon}
                      className="button-main h-12 px-5 bg-black rounded-lg flex items-center justify-center bg-green text-white"
                    >
                      Apply
                    </button>
                  </div>
                      ...
                      <span>
                        {isFreeShippingApplied ? (
                          ...
                          <>
                            ...
                            applied. You save ₹{discountAmount}.00
                          </>
                        )}
                      </span>
                      <button
                  ...
                  <div className="list-payment mt-5">
                    ...
                    >
                      <input
                        ...
                        id="razorpay"
                        name="payment"
                        checked={activePayment === "razorpay"}
                        onChange={() => handlePayment("razorpay")}
                      />
                      <label
                      ...
                      <div className="infor">
                        <div className="text-on-surface-variant1 pt-4">
                          Pay securely using Razorpay. Supports UPI,
            ...
            <div className="xl:w-1/3 xl:pl-12 w-full mt-10 md:mt-0">
              <div className="checkout-block bg-surface  rounded-2xl">
                ...
                {buyNow === "true" && (
                  <div className="text-sm text-green mb-3  bg-green/10 rounded-lg">
                    ...
                    displayCartItems.map((product) => (
                      ...
                      >
                        ...
                        <div className="flex flex-col justify-between w-full">
                          <div>
                            <div className="name text-title line-clamp-1 md:line-clamp-2">
                              {product.name}
                            </div>
                            ...
                            {(product as any).taxId && (
                              <div className="mt-1">
                                {taxLoadingCache[(product as any).taxId] ? (
                                  ...
                                  <span className="caption1 text-secondary">
                                    Price includes{" "}
                                    {taxDataCache[(product as any).taxId].rate}%
                                    GST
                                  </span>
                                ) : null}
                              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-30 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-8 max-w-sm mx-4 text-center shadow-2xl">
            ...
            <p className="text-gray-600 text-sm">
              Processing your order and redirecting you to the success page
            </p>
          </div>
        </div>
      )}
