    ...
    const fetchVendorsData = async () => {
      try {
        ...
        vendorsData.forEach((vendor) => {
          map[vendor.id] =
            vendor.businessName || vendor.storeName || vendor.name || vendor.id;
        });
        setVendorMap(map);
      } catch (error) {
  ...
  let filteredData = data.filter((product) => {
    ...
    if (category) {
      ...
      isDataCategoryMatched =
        prodCat === category ||
        (typeof prodCat === "string" &&
          prodCat.toLowerCase() === category.toLowerCase());
    }
    ...
    if (type) {
      ...
      isTypeMatched =
        productSubCategories.includes(type) || productSubCategory === type;
    }
    ...
    <>
      <div className="breadcrumb-block style-img">
        <div className="breadcrumb-main bg-white overflow-hidden">
          <div className="container lg:pt-[50px] pt-5 relative">
            <div className="main-content w-full h-full flex flex-col items-start justify-center relative z-[1]">
              <div className="text-content flex flex-col items-start">
                <div className="heading2 text-center mt-2">
                  {vendorId ? vendorName : categoryName}
                </div>
                <div className="link flex items-center justify-center gap-1 caption1">
                  <Link href={"/"}>Homepage</Link>
                  <Icon.CaretRight size={14} className="text-secondary2" />
                  <Link href={"/shop/breadcrumb1"} className="text-secondary2">
                    Shop
                  </Link>
                  <Icon.CaretRight size={14} className="text-secondary2" />
              ...
              <div className="list-tab flex overflow-scroll hide-scrollbar  items-start justify-start gap-y-5 gap-5 lg:mt-[32px] mt-8 ">
                {subCategories.length > 0 ? (
                  subCategories.map((sub) => (
                    <Link
                      key={sub.id}
                      href={`/shop/breadcrumb1?categoryId=${encodeURIComponent(
                        categoryId || ""
                      )}&subCategory=${encodeURIComponent(sub.id)}`}
                      className={`flex-shrink-0 bg-black p-2 px-6 rounded-full text-white  text-button-uppercase cursor-pointer  `}
                    >
                      {sub.name}
                    </Link>
                  ))
        ...
        <div className="container">
          <div className="flex max-md:flex-wrap max-md:flex-col-reverse gap-y-8">
                            ...
                            (
                            {
                              data.filter((dataItem) => {
                                const productSubCategories =
                                  (dataItem as any).subCategories || [];
                                const productSubCategory = (dataItem as any)
                                  .subCategory;
                                return (
                ...
                <div className="list-brand mt-4">
                  {allBrands.length > 0 ? (
                    allBrands.map((brand, index) => (
                      <div
                        key={index}
                        className="brand-item flex items-center justify-between mt-3"
                      >
                        <div
                        ...
                        <div className="text-secondary2">
                          ...
                          {(() => {
                            const matchingProducts = data.filter((dataItem) => {
                              const productBrands =
                                (dataItem as any).brands || [];
                              const matches = productBrands.includes(brand.id);
                              console.log(
                                `Product "${dataItem.name}" brand check for "${brand.name}" (${brand.id}):`,
                                {
                                  productBrands,
                                  brandId: brand.id,
                                  matches,
                            ...
                            console.log(
                              `Total products for brand "${brand.name}":`,
              ...
              <div className="filter-style pb-8 border-b border-line mt-8">
                ...
                <div className="list-style mt-4">
                  {[
                    ...
                    <div
                      key={index}
                      className="style-item flex items-center justify-between mt-3"
                    >
                      <div
                        className="left flex items-center cursor-pointer"
                      ...
                      >
                        ...
                        <label
                          htmlFor={style}
                          className="style-name capitalize pl-2 cursor-pointer"
                        >
                          {style}
                        </label>
                      </div>
                      <div className="text-secondary2">
                        (
                        {
                          data.filter((dataItem) => {
                            ...
                            return productStyles.some(
                              (productStyle: string) =>
                                productStyle.toLowerCase() ===
                                style.toLowerCase()
                            );
                          }).length
                        }
              ...
              <div className="filter-material pb-8 border-b border-line mt-8">
                ...
                <div className="list-material mt-4">
                  {[
                    ...
                    <div
                      key={index}
                      className="material-item flex items-center justify-between mt-3"
                    >
                      <div
                        className="left flex items-center cursor-pointer"
                      ...
                      >
                        ...
                        <label
                          htmlFor={material}
                          className="material-name pl-2 cursor-pointer"
                        >
                          {material}
                        </label>
                      </div>
                        ...
                        (
                        {
                          data.filter((dataItem) => {
                            const productMaterials =
                              (dataItem as any).primaryMaterial || [];
                      ...
                      >
                        ...
                        <div className="text-secondary2">
                          (
                          {
                            data.filter(
                              (dataItem) =>
                                (dataItem as any).vendor === vendorId
                            ).length
                          }
                          )
                        </div>
            ...
            <div className="list-product-block lg:w-3/4 md:w-2/3 w-full md:pl-3">
              <div className="list-filtered flex flex-col  items-start gap-3 ">
                ...
                {(selectedType ||
                  ...
                  <>
                    ...
                    <div
                      className="clear-btn flex bg-red- items-center px-2 py-1 gap-1 rounded-full border border-red cursor-pointer"
