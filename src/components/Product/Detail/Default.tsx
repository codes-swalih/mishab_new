...
const Default: React.FC<Props> = ({ data, productId }) => {
  ...
  const fetchCategoryData = useCallback(
    async (categoryId: string, retryCount = 0) => {
      try {
  ...
  const fetchBrandData = useCallback(
    async (brandId: string, retryCount = 0) => {
      try {
  ...
  const fetchVendorData = useCallback(
    async (vendorId: string, retryCount = 0) => {
      try {
      ...
      if (snapshot.exists()) {
        ...
        snapshot.forEach((childSnapshot) => {
          ...
          const product: ProductType = {
            ...
            sold: 0,
            sizes: Array.isArray(productData.attributes?.sizes)
              ? productData.attributes.sizes
              : ["S", "M", "L", "XL"],
            variation: Array.isArray(productData.attributes?.variations)
              ? productData.attributes.variations
              : [],
        ...
        const related = allProducts
          .filter((product) => {
            ...
            const isEnabled =
              product.status !== "disabled" && product.status !== "false";
  ...
  const productMain: ProductType =
    (data.find((product) => product.id === productId) as ProductType) ||
    (data[0] as ProductType);
    ...
    async (productId: string, retryCount = 0) => {
      try {
        ...
        if (retryCount < 2) {
          ...
          setAttributesError(
            "Unable to load product tags. Please try again later."
          );
          setAttributesData([]);
        }
      }
  ...
  const percentSale = Math.floor(
    100 - (unitPrice / productMain?.originPrice) * 100
  );
  ...
  useEffect(() => {
    ...
    if (productMain?.categories && productMain.categories.length > 0) {
      console.log(
        "✅ Found categories, fetching category:",
        productMain.categories[0]
      );
          ...
          <div className="md:ml-10 flex justify-between gap-t- flex-wrap">
            <div className=" list-img md:w-1/2 md:pr-[45px] w-full">
              ...
              <Swiper
                ...
                modules={[Thumbs, Pagination, Autoplay, Navigation]}
              ...
              >
                {productMain.images.map((item, index) => (
                  <SwiperSlide
                    key={index}
                    onClick={() => {
                      // slideToLoop ensures correct slide index when using loop
                      swiperRef.current?.slideTo(index);
              ...
              {/* POPUP / LIGHTBOX SWIPER (loop + autoplay left + pagination) */}
              ...
              {/* Styles to center pagination dots and style popup */}
              <style jsx>{`
                :global(.mySwiper2 .swiper-pagination),
                :global(.popupSwiper .swiper-pagination) {
                  bottom: 12px !important;
                  left: 0;
                  width: 100%;
                  text-align: center;
                }
                :global(.mySwiper2 .swiper-pagination-bullet),
                :global(.popupSwiper .swiper-pagination-bullet) {
                  opacity: 1;
            ...
            <div className="container product-infor rounded-[1rem] bg-white -mt-3 z-10 pt-10 md:-mt-0 md:pt-0 md:w-1/2 w-full lg:pl-[15px] md:pl-2">
              <div className="flex justify-between ">
                <div className=" w-5/6 md:w-ful">
                  <div className="caption2 l  text-secondary font-semibold uppercase">
                    {categoryError ? (
              ...
              <div className="list-action mt-6">
                  ...
                  <span className="text-sm text-secondary">
                    ...
                    <span
                      className={`font-medium ${
                        productMain.quantity > 0 ? "text-green" : "text-red"
                      }`}
                    >
                      {productMain.quantity > 0
                        ? `${productMain.quantity} available`
                        : "Out of stock"}
                    </span>
                  </span>
                </div> */}
                <div className="choose-quantity flex items-center lg:justify-between gap-5 gap-y-3 mt-3">
                  <div
                    className={`quantity-block md:p-3 max-md:py-1.5 max-md:px-3 flex items-center justify-between rounded-lg border border-line sm:w-[180px] w-[120px] flex-shrink-0 ${
                  ...
                  >
                    ...
                    <Icon.Plus
                      ...
                      className={`${
                        productMain.quantityPurchase >= productMain.quantity ||
                        productMain.quantity === 0
                          ? "disabled"
                          : ""
                      } ${
                  ...
                  <div
                    onClick={
                      productMain.quantity > 0 ? handleAddToCart : undefined
                    }
                ...
                <div className="button-block mt-5">
                  <div
                    onClick={
                      productMain.quantity > 0 ? handleBuyNow : undefined
                    }
