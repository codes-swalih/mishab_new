...
import dum from "../../../public/dummyCat.webp";
...
const CategoriesGrid = () => {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const filtered = result
        .filter(
          (c) => c.showInMainMenu === true || c.showInMainMenu === undefined
        )
        .slice(0, 12);
  ...
  return (
    <section className=" container  md:pt-14 ">
      ...
      {loading ? (
        ...
        //     ))}
        // </div>
        <div className=" w-full grid grid-cols-5  gap-3 gap-y-5 hide-scrollbar mb-5">
            ...
            return (
              ...
              >
                <div className=" flex flex-col gap-2  items-center">
                  <div className=" w-[4.1rem] md:w-[10rem]  flex items-center justify-center flex-shrink-0 md:h-[10rem] bg-gray-100 rounded-lg border shadow-md h-[4.1rem]">
                    <Image
                      src={cat.image ? cat.image : ""}
                      width={500}
                      height={500}
                      alt="dummy"
                      className=" w-[2.5rem] h-[2.5rem] md:w-[5rem] md:h-[5rem] object-cover rounded-full bg-gray-200"
                    />
                  </div>
                  <h1 className=" text-xs md:text-base w-10 md:w-full font-medium text-center">
                    {cat.name}
                  </h1>
                </div>
              </Link>
