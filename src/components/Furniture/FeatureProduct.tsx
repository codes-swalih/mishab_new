...
const FeatureProduct: React.FC<Props> = ({ start = 0, limit = 8 }) => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          if (featuredProducts.length === 0) {
            let trendingProducts = enabledProducts.filter(
              (product) => product.trending === true
            );

            // If no trending products, try alternative field names
            if (trendingProducts.length === 0) {
              trendingProducts = enabledProducts.filter(
                (product) =>
            ...
            if (trendingProducts.length === 0) {
              console.log(
                "🔥 No featured or trending products, showing all enabled products"
              );
              finalProducts = enabledProducts;
              setIsShowingTrending(false);
            } else {
              console.log(
                "🔥 No featured products, using trending products:",
        ...
        <div className="container">
          ...
          <div className="list-product hide-product-sold grid lg:grid-cols-4 grid-cols-2 sm:gap-[30px] gap-[20px] md:mt-10 mt-6">
            {[1, 2, 3, 4].map((_, index) => (
              <div
                key={index}
                className="animate-pulse bg-gray-200 rounded-2xl aspect-[3/4]"
              ></div>
            ))}
          </div>
        </div>
  ...
  if (error) {
    return (
      <div className="whate-new-block md:pt-20 ">
        <div className="container">
          <div className="heading flex flex-col md:items-center md:text-center">
  ...
  if (products.length === 0) {
    return (
      <div className="whate-new-block md:pt-20 ">
        <div className="container">
          <div className="heading flex flex-col items-start text-start">
            <div className="heading3">Featured Products</div>
            <div className="text-secondary mt-4">
  ...
  console.log(
    "🔄 Render - Display products after conversion:",
    ...
    <>
      <div className=" md:pt-20">
        <div className="container">
          ...
          <div className="list-product hide-product-sold grid lg:grid-cols-4 grid-cols-2 sm:gap-[30px] gap-[20px] md:mt-10 mt-4">
